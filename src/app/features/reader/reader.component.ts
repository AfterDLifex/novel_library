...
export class ReaderComponent implements OnInit, OnDestroy {
  ...
  // Declare first, initialize inside constructor
  readonly fontSize;
  readonly theme;
  ...
  async ngOnInit() {
    const idParam =
      this.route.snapshot.paramMap.get('novelId') ?? '';

    const chParam =
      this.route.snapshot.paramMap.get('chapterId') ?? '';

    this.novelIdVal =
      idParam.startsWith('novel:')
        ? idParam
        : `novel:${idParam}`;
  ...
  async loadChapters() {
    ...
    try {
      const all = await this.coordinator.getChapters(
        this.novelIdVal
      );
    ...
    if (progress && this.contentEl) {
      setTimeout(() => {
        ...
        if (
          ...
          el.scrollHeight > progress.scrollPosition
        ) {
